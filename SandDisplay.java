  ...
  MouseMotionListener, ActionListener, ChangeListener
{
  ...
  public SandDisplay(String title, int numRows, int numCols, String[] buttonNames)
  {
    ...
    //determine cell size
    cellSize = Math.max(1, 950 / Math.max(numRows, numCols));
    ...
    brushSlider = new JSlider(JSlider.HORIZONTAL, 1,40,20);
    brushSlider.addChangeListener(this);
    brushSlider.setMajorTickSpacing(1);
    brushSlider.setPaintTicks(true);
    Hashtable<Integer,JLabel> brushTable = new Hashtable<Integer,JLabel>();
    brushTable.put(new Integer(1),new JLabel("1"));
    brushTable.put(new Integer(40),new JLabel("40"));
    brushSlider.setLabelTable(brushTable);
    brushSlider.setPaintLabels(true);


    frame.getContentPane().add(slider);
    frame.getContentPane().add(brushSlider);

    frame.pack();
    frame.setVisible(true);
  }
  public int getBrushSize() { return brushSlider.getValue(); }
